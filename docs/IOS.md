...
### Замусорит ли Xcode репозиторий?

Копировать папку никуда не нужно — работайте прямо в репозитории. Мусор Xcode
(`xcuserdata/`, `DerivedData/`, результаты сборки) уже в `.gitignore` и в
коммиты не попадёт. `xcuserdata` важен отдельно: он содержит **имя пользователя
Мака**, поэтому его нельзя публиковать.

Единственное, что Xcode меняет по-настоящему, — это `project.pbxproj`, когда вы
выбираете свою команду в **Signing & Capabilities**. Это изменение **не
коммитьте**: там будет ваш Team ID. Проверить перед коммитом:

```bash
git status
```

Чтобы вообще не трогать файл проекта, передавайте команду сборке аргументом
(способ из шага 2) вместо выбора в интерфейсе.
