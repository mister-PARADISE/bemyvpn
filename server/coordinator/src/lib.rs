...
async fn register_host(
    ...
    // Публиковать нечего: внешний адрес не определился.
    //
    // ЗДЕСЬ НЕЛЬЗЯ ПИСАТЬ «ВЫ ЗА NAT». За NAT раздавать МОЖНО — ради этого и
    // сделано пробитие: мы видим хост по внешнему адресу его же NAT, он
    // публичный, и запись публикуется как обычно. Прежний текст утверждал
    // обратное, то есть отговаривал людей от того, ради чего продукт и написан.
    //
    // Сюда попадают ДРУГИЕ случаи: наблюдаемый адрес приватный (хост и
    // координатор в одной локальной сети), адрес не разобрался, либо сеть не
    // даёт стабильного внешнего порта вовсе. Причина человеку неизвестна и
    // неинтересна — ему нужен следующий шаг, а не диагноз.
    if ann.endpoints.is_empty() {
        state.rej_nat.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        return (
            StatusCode::UNPROCESSABLE_ENTITY,
            "Не удалось определить ваш адрес в интернете — без него гости не найдут дорогу. Попробуйте ещё раз, а если не проходит — из другой сети.",
        );
    }
...
mod tests {
    ...
    async fn rejections_explain_themselves_and_are_counted() {
        ...
        assert!(
            why.contains("адрес в интернете"),
            "причина отказа не названа человеческими словами: {why:?}"
        );
        // И отдельно: НЕ смеем утверждать, что дело в NAT. За NAT раздавать можно —
        // ради этого сделано пробитие, — и прежний текст отговаривал людей от того,
        // ради чего продукт написан.
        assert!(!why.contains("NAT"), "отказ снова винит NAT: {why:?}");
