...
//!
//! ЖИЛ В ДВОИЧНОМ КРЕЙТЕ ОКНА, и терминал до него не дотягивался: `lib`-цели у
//! окна нет, позвать снаружи нечем. Терминал из-за этого показывал пустую
//! колонку там, где три другие оболочки показывали «NL» и «US». Разбор переехал
//! сюда целиком, а обе оболочки его ЗОВУТ — второй копии разбора не заведено.
//!
//! База (~490 КБ) вшита в бинарь, поэтому модуль закрыт фичей `geoip`: у
//! телефонов своя копия в ресурсах, и мосту (`bmv-ffi`) этот вес не нужен.
//! Само ПРАВИЛО показа («страна по адресу, объявленное поле — запасным») лежит
//! не здесь, а в справочнике правил: `view::host_country`.
...
mod tests {
    ...
    #[test]
    fn database_is_not_coarse() {
        // Прежняя база шла по стране РАСПРЕДЕЛЕНИЯ блока и сливала соседние
        // диапазоны: этот адрес значился US, хотя сервер физически в Заандаме.
        // Тест ловит откат к грубой базе при следующем обновлении.
        assert_eq!(country_of("107.189.27.163").as_deref(), Some("NL"));
    }
}
