...
mod tests {
    ...
    /// «не удалось определить ваш адрес в интернете» человек читал сырую строку
    ...
    fn refusal_carries_code_and_shows_only_human_text() {
        let e = refusal(&json!({ "t": "error", "code": 422, "reason": "Не удалось определить ваш адрес в интернете." }));
        assert_eq!(e.refusal_code(), Some(422), "код отказа потерян — ветки в оболочках снова мертвы");
        assert_eq!(e.to_string(), "Не удалось определить ваш адрес в интернете.", "в текст для человека подмешалось лишнее");
