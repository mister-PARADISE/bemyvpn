...
/// Габариты окна: пропорция макета, вписанная в экран.
///
/// Тянуть окно мышью нельзя — макет рассчитан ровно на одну пропорцию (400:820),
/// и растягивание его ломало. Но и прибивать 820 намертво нельзя: на ноутбуке с
/// экраном в 800 точек окно не помещалось и уезжало за нижний край вместе с
/// нав-баром.
///
/// Поэтому высоту берём от ЭКРАНА: 88% его высоты, но не больше 820 (выше макет
/// не становится красивее, просто разъезжается) и не меньше 560 (ниже список
/// хостов вырождается в щель). Оставшиеся 12% — запас на строку меню, панель
/// задач и док: окно во всю высоту упирается в них и выглядит зажатым.
///
/// Ширина ВЫВОДИТСЯ из высоты, а не задаётся: так пропорция сохраняется при
/// любом экране.
fn window_size() -> (f32, f32) {
    const RATIO: f32 = 400.0 / 820.0;
    const IDEAL_H: f32 = 820.0;
    const MIN_H: f32 = 560.0;
    const SCREEN_SHARE: f32 = 0.88;

    // Экран не удалось спросить — берём идеал: на большинстве машин он влезает,
    // а на маленьких ОС всё равно подожмёт окно сама.
    ...
    let h = (screen_h * SCREEN_SHARE).min(IDEAL_H).max(MIN_H);
...
mod tests {
    ...
    /// Окно вписывается в экран, сохраняя пропорцию макета.
    ///
    /// Проверяем ту самую арифметику, из-за которой окно уезжало за нижний край:
    /// на низком экране высота ОБЯЗАНА ужаться, а ширина — поехать за ней, иначе
    /// пропорция ломается и макет разъезжается.
    #[test]
    fn window_keeps_its_proportion_and_fits_the_screen() {
        const RATIO: f32 = 400.0 / 820.0;
        // Та же формула, что в window_size, но с подставленным экраном.
        let fit = |screen_h: f32| -> (f32, f32) {
            let h = (screen_h * 0.88).min(820.0).max(560.0);
            ((h * RATIO).round(), h.round())
        };

        // Ноутбук 1280×800 — тот случай, на котором окно не помещалось.
        let (w, h) = fit(800.0);
        assert!(h < 800.0, "окно обязано быть НИЖЕ экрана, а не вровень");
        assert!(h <= 820.0 && h >= 560.0);
        assert!((w / h - RATIO).abs() < 0.01, "пропорция должна сохраняться");

        // Большой экран — выше идеала не растём: макет от этого не выигрывает.
        let (_, h_big) = fit(1400.0);
        assert_eq!(h_big, 820.0);

        // Совсем низкий экран — не схлопываемся ниже разумного.
        let (_, h_tiny) = fit(500.0);
        assert_eq!(h_tiny, 560.0);
    }
