...
///
/// НА WINDOWS ОНА НА 40 ТОЧЕК МЕНЬШЕ, И ЭТО ВЫРАВНИВАНИЕ ОБОЛОЧЕК, А НЕ
/// РАСХОЖДЕНИЕ. Человек сказал: на маке ширина устраивает, на всех Windows окно
/// широкое. Прежде чем трогать число, платформенную причину искали замером на
/// настоящих машинах (обе архитектуры, job `gui-screenshot-windows`), и первые
/// две догадки оказались НЕВЕРНЫ:
///
///  * рамку окно не добавляет: клиентская область ровно 400×824 точки, видимая
///    рамка 1 точка на сторону, заголовок 32 (строки «MEASURE» в логе задачи);
///  * точки с пикселями мы не путаем: при масштабах 100/125/150/200 % клиент
///    выходит 400/500/600/800 пикселей — то есть РОВНО 400 точек на каждом
///    (строки «SCALE», проверка SLINT_SCALE_FACTOR).
///
/// А вот что действительно различается — ШИРИНА СИСТЕМНОГО ШРИФТА. Один и тот же
/// абзац в одной и той же колонке 360 точек («Через ваш канал выйдет в сеть
/// тот…», кегль 13): на маке первая строка кончается словом «закрыли», на
/// Windows в неё влезает ещё и «доступ.». По двум первым строкам это 105 знаков
/// против 93 — Segoe UI уже SF Pro примерно на 13 %. Колонка та же, а текста в
/// ней больше и пустоты по краям больше: ровно то, что читается как «шире, чем
/// хотелось бы». Чтобы СТРОКА БЫЛА ОДНОЙ ДЛИНЫ в знаках, колонка на Windows
/// должна быть 400 / 1.13 ≈ 354 точки.
///
/// Второе, в ту же сторону: у Windows при масштабе логический стол ЗАМЕТНО уже
/// (FullHD при 125 % — 1536 точек, при 150 % — 1280) против 1710 у мака. Те же
/// 400 точек занимают там 26–31 % ширины экрана вместо 23 %.
///
/// 360, а не 354, — потому что 360 ещё и проверено снизу: см. `LAYOUT_W_MIN`.
const LAYOUT_W: f32 = if cfg!(windows) { 360.0 } else { 400.0 };

/// Ниже этого макет ЛОМАЕТСЯ — предел найден на живом окне, а не на глаз.
///
/// Первой упирается НЕ строка плиток лимита гостей (на неё думали в первую
/// очередь): при 340 точках плитка «128» получает 45 точек при 23 на саму
/// надпись и держится примерно до 230. Первой сдаётся плитка «ПРОТОКОЛ» в
/// парящей панели: слово «Маскировка» при 340 ещё целиком, при 320 уже
/// «Маскиро…». Считается это так: плитка = (W − 84) / 2, на текст остаётся
/// плитка − 39 (отступы 22, зазор 5, значок 12), а «Маскировка» занимает от 79
/// до 89 точек — значит перелом лежит между 320 и 340.
///
/// Второй предел рядом: код сети из 12 знаков (кегль 26, моноширинный, разрядка
/// 2) занимает 223 точки, а панель отдаёт под него W − 76 — то есть код упрётся
/// в края при 299.
///
/// 360 оставляет над обоими запас в 20 и 60 точек соответственно.
const LAYOUT_W_MIN: f32 = 360.0;

/// Сторож на самой сборке: проектная ширина не может опуститься ниже предела,
/// на котором макет начинает обрезаться. Тестом это было бы слабее — тест можно
/// не запустить, а компиляцию не обойти.
const _: () = assert!(
    LAYOUT_W >= LAYOUT_W_MIN,
    "ширина макета ниже измеренного предела: в парящей панели обрежется название протокола"
);
...
mod tests {
    ...
    fn window_is_always_layout_wide_and_fits_the_screen_in_height() {
        ...
        // Число зависит от оболочки — и это НЕ расхождение, а поправка на
        // ширину системного шрифта; разбор у самой константы.
        assert_eq!(LAYOUT_W, if cfg!(windows) { 360.0 } else { 400.0 });
    ...
    fn the_window_looks_the_same_at_every_screen_scale() {
        ...
        for (pixels, scale) in [(1080.0_f32, 1.0_f32), (1080.0, 1.25), (1080.0, 1.5), (2160.0, 2.0)] {
            ...
            assert_eq!(window_size().0, LAYOUT_W, "масштаб {scale}: ширина макета уехала");
