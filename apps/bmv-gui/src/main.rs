...
/// Габариты окна: 75% высоты экрана, пропорция макета сохранена.
///
/// Тянуть окно мышью нельзя — макет рассчитан ровно на одну пропорцию (400:820),
/// растягивание его ломает. Но и прибивать 820 намертво нельзя: на ноутбуке с
/// экраном в 800 точек окно не помещалось и уезжало за нижний край.
///
/// Доля именно 75%: оставшаяся четверть — это и системные панели (строка меню и
/// док на macOS, панель задач на Windows), и просто воздух. Окно впритык к краю
/// читается как «не поместилось», даже когда поместилось.
///
/// Считаем от ПОЛНОЙ высоты окна, вместе с заголовком: его рисует ОС поверх
/// запрошенной высоты, и без этой поправки окно на экране всегда выше
/// заказанного — ровно на высоту заголовка.
///
/// Ширина ВЫВОДИТСЯ из высоты, а не задаётся: так пропорция держится на любом
/// экране.
...
mod tests {
    ...
    /// Окно занимает три четверти экрана — вместе со своим заголовком.
    ///
    /// Первая версия брала долю от высоты СОДЕРЖИМОГО и не считала заголовок:
    /// на экране окно выходило выше заказанного и упиралось в панель задач.
    /// Тест меряет именно то, что видно на экране, а не то, что мы попросили.
    #[test]
    fn window_takes_three_quarters_of_the_screen_including_its_title() {
        const TITLE_BAR: f32 = 40.0;
        const RATIO: f32 = 400.0 / 820.0;
        ...
        for screen in [800.0_f32, 1080.0, 1107.0, 1440.0] {
            ...
            assert!(
                (share - 0.75).abs() < 0.02,
                "экран {screen}: окно занимает {on_screen} — это {share} экрана, а не три четверти",
            );
            // Четверть экрана остаётся системным панелям и воздуху.
            assert!(screen - on_screen > 150.0, "экран {screen}: слишком тесно");
        }

        // Пропорция держится на любом экране.
        for screen in [800.0_f32, 1440.0] {
            let h = fit_height(screen);
            let w = (h * RATIO).round();
