...
fn elevate_launch(exe: &std::path::Path, port_file: &std::path::Path, token_file: &std::path::Path, up_file: &std::path::Path, cfg_file: &std::path::Path) -> Result<(), String> {
    ...
    // ЗДЕСЬ СТОЯЛО «Нужен пароль администратора» — и это была та же ложь, что в
    // гостевом слое (см. `TunFail` в bmv-desktop), только злее: `spawn` падает ДО
    // того, как у человека что-то спросили, поэтому пароль тут ни при чём вообще.
    // Настоящая причина — pkexec в системе не установлен (нет polkit): на таком
    // дистрибутиве человек вечно читал бы про пароль и никогда не догадался бы
    // доставить пакет.
    cmd.spawn().map(|_| ()).map_err(|e| {
        if e.kind() == std::io::ErrorKind::NotFound {
            "Не удалось запросить права администратора: в системе нет pkexec (пакет polkit). \
             Установите его — без него окно с паролем не появится."
                .to_string()
        } else {
            // Причина другая, а какая — не знаем. Так и говорим.
            "Не удалось запустить помощника VPN. Попробуйте ещё раз.".to_string()
        }
    })
}
