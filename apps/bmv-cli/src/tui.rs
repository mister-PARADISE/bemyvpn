...
///
/// В поле каталога лежит то, что о себе объявил ХОСТ, а по умолчанию там стоит
/// служебное `auto` — «определи сам» (`bmv_config`, `country_hint`). Терминал
/// печатал это поле как есть и показывал человеку служебное слово в каждой
/// строке. Три другие оболочки страну ВЫЧИСЛЯЮТ по адресу, а объявленное поле
/// берут запасным вариантом и только если в нём настоящий код — две латинские
/// буквы. Не определилась — не показываем НИЧЕГО.
///
/// ОПРЕДЕЛЕНИЯ ПО АДРЕСУ ЗДЕСЬ ПОКА НЕТ, и это честно названо: разбор базы
/// IP→страна написан (`apps/bmv-gui/src/geo.rs`), но лежит в ДВОИЧНОМ крейте
/// окна — позвать его из терминала нечем. Вторая копия разбора и есть то самое
/// расхождение, ради которого заведён `bmv_common::view`, поэтому её тут не
/// будет: переедет разбор в общий крейт — здесь появится первая строка
/// `geo::country_of(&h.ip).or_else(…)`, и больше ничего не изменится.
