...
enum Cmd {
    ...
    Host {
        /// Пропускать трафик гостей наружу по-настоящему — быть их выходом в
        /// интернет. Root НЕ нужен: хост работает в userspace (обычные сокеты).
        /// Без флага — эхо-проверка.
        #[arg(long)]
        tunnel: bool,
...
async fn main() {
    ...
    if matches!(cmd, Cmd::Host { .. }) && (config.host.id.is_empty() || config.host.code_sig.is_empty()) {
        ...
        match tmp.host_new_code().await {
            ...
            // Сервер объяснил отказ — печатаем ЕГО слова: заготовка про связь
            // уходила и тому, у кого просто не задан адрес координатора.
            Err(e) => fail(e.to_string()),
            Ok(_) => fail("сервер не выдал код сети — без него раздавать нельзя. Проверьте связь и повторите".into()),
        }
    }
...
fn run_install(dir: Option<std::path::PathBuf>) {
    ...
    println!("Дальше: `bemyvpn` — меню, `bemyvpn host` — стать чьим-то VPN, `bemyvpn update` — обновиться.");
}
