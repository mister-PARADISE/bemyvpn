...
///
/// ПУСТАЯ КОЛОНКА-ЗНАЧОК СХЛОПЫВАЕТСЯ. Страна и замок есть не у всех, а сегодня
/// страны нет ни у кого (см. `host_cc` в `tui.rs`): держать под них место
/// значило бы отодвинуть каждое имя на шесть пустых клеток вправо — ровно та
/// пустота, из-за которой список и выглядел неряшливо. Ширины считаются на весь
/// список сразу, поэтому строки всё равно стоят в столбец.
