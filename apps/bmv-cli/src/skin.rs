...
/// Значок состояния: эмодзи — две клетки.
const COL_DOT: usize = 2;
/// Код страны — две латинские буквы.
pub const COL_CC: usize = 2;
/// Замок: эмодзи — две клетки.
pub const COL_LOCK: usize = 2;
/// Гости: «120/120».
pub const COL_GUESTS: usize = 7;
/// Протокол: значок (2) + пробел + самая длинная подпись справочника (10).
pub const COL_PROTO: usize = 13;
/// Отклик: «1234 мс».
pub const COL_PING: usize = 7;
...
///
/// ПУСТАЯ КОЛОНКА-ЗНАЧОК СХЛОПЫВАЕТСЯ. Страна и замок есть не у всех (страна не
/// определяется по приватному адресу, см. `host_cc` в `tui.rs`): держать под них
/// место значило бы отодвинуть каждое имя на шесть пустых клеток вправо — ровно
/// та пустота, из-за которой список и выглядел неряшливо. Ширины считаются на
/// весь список сразу, поэтому строки всё равно стоят в столбец.
pub fn host_row_fixed(cc: bool, lock: bool) -> usize {
    2 + COL_DOT
        + 1
        + if cc { COL_CC + 1 } else { 0 }
        + if lock { COL_LOCK + 1 } else { 0 }
        + 1
        + COL_GUESTS
        + 1
        + COL_PROTO
...
/// иначе шапка и строки разъедутся первым же значком. `left` — сколько клеток
/// занимают колонки-значки слева (у них подписи нет: значок сам себе подпись).
pub fn host_head(left: usize, name_w: usize) -> Line<'static> {
    hint(format!(
        "{:left$}{} {} {} {}",
        "",
        cell("ХОСТ", name_w),
