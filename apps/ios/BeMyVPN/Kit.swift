...
/// гостей.
/// КОДА СТРАНЫ здесь тоже НЕТ. Он стоял первым («NL · гостей 0/32») и повторял
/// флаг, который лежит слева на аватарке в сантиметре от него: одна и та же
/// страна дважды в одной строке. Флаг остаётся, а место уходит счётчику. Полный
/// код виден в раскрытой карточке, в плитке «СТРАНА».
func hostSubtitle(_ h: Host) -> Text {
    // Потолок хост может и не объявить (0) — дробь «1/0» в этом случае врёт.
    Text(h.max > 0 ? "гостей \(h.guests)/\(h.max)" : "гостей \(h.guests)")
}
