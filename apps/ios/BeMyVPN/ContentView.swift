...
struct HostCard: View {
    ...
    var body: some View {
        ...
        // подкраска (picked) — на «выбрано ли». Подключён при этом может быть
        // ОДИН хост, а раскрыт совсем ДРУГОЙ, и увидеть надо оба сразу.
        // Поэтому «подключено» ушло на контур: мята в полную силу (edge) и
        // ВДВОЕ ТОЛЩЕ (2 против 1), плюс чип «Подключено» в строке имени.
        // Признак не один цвет: толщина и слово переживают любое нарушение
        // цветовосприятия.
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(expanded ? Theme.cardHi : Theme.card)
                .overlay(RoundedRectangle(cornerRadius: 16, style: .continuous)
                    .stroke(live ? Theme.edge() : (expanded ? Theme.edgeSoft() : Theme.hairline),
                            lineWidth: live ? 2 : 1))
        )
