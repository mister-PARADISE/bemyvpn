...
/// Ширина завесы под парящим блоком: содержимое, уходящее под него, ГАСНЕТ, а
/// не срезается ножом. Отступы прокрутки считаются ОТ НЕЁ — покоящееся
/// содержимое не должно попадать в полосу гашения, иначе завеса выглядит тенью
/// на пустом месте.
...
func floatSurface(radius: CGFloat, stroke: Color, up: Bool = false) -> some View {
    RoundedRectangle(cornerRadius: radius, style: .continuous)
        ...
        //
        // Раньше список обрывался о кромку панели ножом. Полоса в цвет
        // страницы, сходящая на нет от края блока наружу, превращает обрыв в
        // уход. Живёт ЗДЕСЬ намеренно: одна правка накрывает и панель состояния
        // (все три вкладки), и нав-бар — как `FloatCard` на десктопе.
        //
        // Стоит ПОСЛЕ `.shadow`, иначе тень легла бы и на саму завесу.
        //
        ...
        .overlay(alignment: up ? .top : .bottom) {
            LinearGradient(
                colors: up ? [Theme.bg.opacity(0), Theme.bg] : [Theme.bg, Theme.bg.opacity(0)],
                startPoint: .top, endPoint: .bottom
            )
            .frame(height: veilWidth)
            .offset(y: up ? -veilWidth : veilWidth)
            .allowsHitTesting(false)
        }
        // ПОЛОСКА ПОД БАРОМ. Он приподнят над нижним краем, и в этот просвет
        // пролезали обрывки карточек — из-под бара торчала половина строки
        // «US · гостей 0/64», срезанная ножом. Ничего полезного там нет и быть
        // не может. Гасить нечего — просто фон, и градиент не нужен: снизу
        // полосу обрезает сам край экрана.
        //
        // Сверху такой же полосы ЗДЕСЬ НЕТ, и это не забывчивость: панель живёт
        // в `safeAreaInset`, а он обрезает всё, что вылезает за его край, —
        // проверено на симуляторе, полоса просто не рисовалась. Верхний просвет
        // закрыт в `ContentView.body`, где обрезать некому.
        .overlay(alignment: .bottom) {
            if up {
                Theme.bg.frame(height: 60).offset(y: 60).allowsHitTesting(false)
            }
        }
}
