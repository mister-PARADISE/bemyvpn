        ...
        Thread {
            ...
            val pfd = try {
                Builder()
                    ...
                    .addRoute("0.0.0.0", 0)
                    // IPv6 МАРШРУТА ЗДЕСЬ НЕТ — И ЭТО НАРОЧНО, не забывчивость.
                    //
                    // Туннель несёт только IPv4, и на десктопе с iOS такой туннель
                    // обязан сам заглушить v6, иначе тот идёт мимо с настоящим
                    // адресом человека. Android этот случай закрывает САМ: не увидев
                    // у VPN ни одного v6-маршрута, система кладёт в его таблицу
                    // `unreachable default`, и connect() по v6 у всех приложений
                    // падает мгновенно с «No route to host». Проверено на живом
                    // android-35: без ::/0 в таблице tun0 стоит `unreachable default
                    // dev lo`, правило 13000 гонит туда все uid, кроме нашего.
                    //
                    // Добавить сюда `.addRoute("::", 0)` — значит ЗАМЕНИТЬ отвергающий
                    // маршрут системы своим (проверено: `unreachable` исчезает). Утечки
                    // не будет и так, но v6-пакеты пойдут в туннель, где ядро их молча
                    // выбросит (`bmv_tunnel::to_host_allowed`): мгновенный отказ
                    // превратится в чёрную дыру и ожидание таймаута на каждой попытке.
                    // Ровно поэтому и `RouteGuard` на десктопе ставит ОТВЕРГАЮЩИЕ
                    // маршруты, а не «в никуда».
