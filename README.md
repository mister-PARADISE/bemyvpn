...
> ⚠️ Так раздача живёт, **пока открыт терминал**: закроешь SSH или перезагрузишь
> сервер — она погаснет. Чтобы работала всегда, см. следующий пункт (нужен root).

<details><summary>⚙️ <b>Чтобы работал всегда — одна кнопка</b> — клик</summary>

Юниты systemd руками писать не нужно. Одной командой, без всякого меню:

```bash
sudo ./bemyvpn-linux-x86_64-terminal host --tunnel --autostart
```

Служба поставится, запустится и включится в автозагрузку — команда сразу вернёт
управление. Годится для скриптов и первичной настройки сервера.

Ничего больше указывать не нужно: **имя** берётся от машины (у всех разное, а не
«Мой сервер» в десятке экземпляров), **лимит гостей** подбирается по числу ядер —
слабой машине достанется скромное значение, мощному VPS большое. Захочешь иначе —
`--name "…"`, `--max 32`, `--password …`, `--hidden`, или потом в меню.

Либо то же самое руками через меню:
...
**Именно `sudo` тут обязателен:** служба ставится в `/etc/systemd/system/`, куда
обычному пользователю нельзя. Без root та же кнопка поднимет раздачу только на
время, пока открыто меню — перезагрузку она не переживёт.
