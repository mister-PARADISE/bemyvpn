...
# ВЕРСИЯ БЕРЁТСЯ ИЗ ТОЙ ЖЕ ПЕРЕМЕННОЙ, ЧТО И ВНУТРИ БИНАРЯ.
#
# Раньше здесь стояло жёсткое «1.0», и в свойствах приложения всегда значилось
# 1.0 независимо от выпуска — выглядело как заброшенное. Хуже: версия в системе
# расходилась с версией внутри программы, то есть два источника правды об одном.
#
# `BMV_VERSION` — то же, чем помечает себя сам бинарь (см. bmv-common/version.rs):
# CI отдаёт «1.30» из тега «v1.30». Локальная сборка без переменной остаётся
# 0.0-dev и НЕ предлагает себе обновления — это осознанно, разработчику не надо
# подсовывать релиз поверх собственной сборки. Но если локальную сборку отдают
# ЧЕЛОВЕКУ, а не разработчику, переменную надо задать: иначе обновления до него
# не дойдут никогда, и он будет ждать их молча.
VERSION="${BMV_VERSION:-${VERSION:-0.0-dev}}"
