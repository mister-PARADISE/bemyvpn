        ...
        std::collections::hash_map::Entry::Vacant(entry) => {
            ...
            tokio::spawn(async move {
                ...
                if let Err(e) = session_remove_tx.send(network_tuple) {
                    // BeMyVPN fork: штатная гонка при закрытии всего стека
                    // (канал учёта сессий уже закрыт) — не ошибка, а шум.
                    log::debug!("Failed to send session removal for {network_tuple}: {e}");
                }
            });
