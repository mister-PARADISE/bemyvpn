...
// ПРАВКА BeMyVPN: `extern "stdcall"` → `extern "system"` (здесь и в log.rs).
// stdcall существует только на 32-битной Windows; на x86_64 rustc его молча
// принимал за "C" и ругался «not a supported ABI … will become a hard error».
// "system" — это ровно stdcall на i686 и "C" на x86_64/aarch64, то есть код
// тот же, а предупреждения и будущая поломка сборки уходят.
