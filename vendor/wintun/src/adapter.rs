...
/// ПАТЧ BeMyVPN. `netsh` — консольная программа. Запущенная из GUI-процесса
/// (`windows_subsystem = "windows"`, своей консоли нет), она получает от Windows
/// НОВУЮ консоль — и на экране мигает чёрное окно. Апстрим звал `Command::new`
/// без флагов и мигал трижды за подключение. `CREATE_NO_WINDOW` запрещает эту
/// консоль и больше не меняет ничего: stdout/stderr так же ловятся `.output()`.
/// Ровно то же правило, что и в `bmv_common::command` для нашего кода.
fn netsh() -> Command {
